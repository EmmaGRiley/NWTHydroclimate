...
save_path <- paste0(user, file_location, "Figures and Tables/")
...
#For clim_plot_monthly_working -> must edit label in clim_plot_monthly_working.R to the given year
...
Cumulative_flows_current <- df$cum_value[df$CalendarYear == lubridate::year(end_date)]
...
df <- df %>%
  ...
  dplyr::filter(CalendarYear > 1971)
...
hydro_plot_dayofyear_cumulative(station_number = "07OB001",
                                parameter = "flow",
                                select_year = c(2022),
                                after_bennett = FALSE,
                                historic_min = NA,
...
#replace cumulative flows in df using gap-filled values from previous reports
# 2020 -> 141.9 billion m3
#mean cumulative flows from 1972 onwards is 105 billion m3 in 2019. 

mean_annual_flow <- mean(df$cum_value, na.rm=T)

current_annual_flow <- df$cum_value[df$CalendarYear == lubridate::year(end_date)]
